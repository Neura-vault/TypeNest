-- ============================================================
-- 0024: Fix "column reference X is ambiguous" (systematic audit)
--
-- RETURNS TABLE(...) columns become implicit OUT parameters in PL/pgSQL,
-- which are visible as plain identifiers inside the function body. Any
-- place a query used a bare column name that happened to match one of
-- those OUT parameter names, Postgres correctly refused to guess which
-- one was meant. Found and fixed every occurrence of this pattern across
-- all plpgsql functions added so far:
--
--   create_race_room        "code"           -> qualified as r.code
--   join_race_room          "room_id"  (x2)  -> qualified as rp.room_id
--   create_team              "code"           -> qualified as t.code
--   create_tournament        "code"           -> qualified as t.code
--   join_tournament          "tournament_id" (x2) -> qualified as tp.tournament_id
--   finish_race              "place"          -> qualified as rp.place
--   create_tournament_round  "code"           -> qualified defensively (wasn't
--                                                actually ambiguous — this
--                                                function returns void, so it
--                                                has no OUT-parameter table —
--                                                but qualifying it removes any
--                                                doubt for future readers)
--
-- (join_team, leave_team, start_race, submit_test, submit_char_reviews,
-- and every LANGUAGE SQL function were audited too and don't have this
-- issue — LANGUAGE SQL functions don't create the same variable/column
-- collision that LANGUAGE PLPGSQL functions do.)
-- ============================================================
...
begin
  ...
  loop
    ...
    exit when not exists (select 1 from public.race_rooms r where r.code = v_code) or v_attempt > 10;
  end loop;
...
begin
  ...
  if exists (select 1 from public.race_participants rp where rp.room_id = v_room.id and rp.user_id = v_user_id) then
  ...
  select count(*) into v_count from public.race_participants rp where rp.room_id = v_room.id;
...
begin
  ...
  loop
    v_code := upper(substr(md5(random()::text), 1, 6));
    v_attempt := v_attempt + 1;
    exit when not exists (select 1 from public.teams t where t.code = v_code) or v_attempt > 10;
  end loop;
...
begin
  ...
  if exists (
    ...
    where tp.tournament_id = v_tournament.id and tp.user_id = v_user_id
  ) then
  ...
  select count(*) into v_count from public.tournament_participants tp where tp.tournament_id = v_tournament.id;
...
-- create_tournament_round returns void (no OUT-parameter table at all),
-- so it was never actually ambiguous — qualified anyway for consistency.
...
-- Same signature as before — only the "place = 1" line (inside the
-- tournament-resolution block) is now qualified as rp.place, since
-- "place" is also this function's own OUT parameter name.
...
begin
  ...
  if v_finished >= v_total then
    ...
    if v_match_id is not null then
      select rp.user_id into v_match_winner from public.race_participants rp
        where rp.room_id = p_room_id and rp.place = 1;
